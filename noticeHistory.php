...
<?php 
    if (isset($_REQUEST['notyIds'])) {
        $notyId = $_REQUEST['notyIds'];
        $upnoty = $pdo->prepare('UPDATE notification SET status=? WHERE id=?');
        $upnoty->execute(array('read',$notyId));
        header('location:noticeHistory.php');
                      ...
                      <?php 
                       ...
                       ?>
                        <tr>
                            ...
                            <td>
                                <a data-fancybox data-src="#hidden-content<?php echo $row['notice_id'];?>" href="javascript:;<?php echo $row['notice_id'];?>">Notice Details</a>&nbsp;
