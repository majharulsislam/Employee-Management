...
<?php 
    if (isset($_POST['sendnewpass'])) {
        ...
        try {
            ...
            $statement = $pdo->prepare("SELECT * FROM admin_info WHERE admin_pass=?");
            ...
            $admin_id = $_SESSION['logged_in'];
            $new_password = sha1($newpass);
            $statement = $pdo->prepare("UPDATE admin_info SET admin_pass=? WHERE admin_id=?");
            $statement->execute(array($new_password,$admin_id));
